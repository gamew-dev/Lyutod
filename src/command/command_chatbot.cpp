...
void commands::command_chatbot (dpp::cluster& bot, const dpp::slashcommand_t& event) {
    ...
    if (subcommand.name == "check_memory") {
        ...
        if (Config::isLog) std::cout << "pemanggilan fungsi memory..." << std::endl;
        std::string memory = Config::userReadMemory(user_id_str);
        if (memory == "") memory = "Anda belum memiliki history memory";
        if (Config::isLog) std::cout << "memory yang didapat: "<< memory << std::endl;
        event.edit_response("```" + memory + "```");
        if (Config::isLog) std::cout << "edit respon/jawab" << std::endl;
    }
        ...
        if (Config::isLog) std::cout << "ambil data history" << std::endl;
        ...
        for (const auto& line : history) {
            ...
            result += display + "\n";
            if (Config::isLog) std::cout << "cek history line: " << line << std::endl;
        }
